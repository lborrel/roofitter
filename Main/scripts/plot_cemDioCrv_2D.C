void plot_cemDioCrv_2D(std::string filename) {

  TFile* file = new TFile(filename.c_str(), "READ");
  
  RooWorkspace* ws = (RooWorkspace*) file->Get("cemDioCrv_2D/cemDioCrv_2D");
  ...
  pdf->plotOn(plot_mom, RooFit::LineColor(kBlack));
  pdf->plotOn(plot_t0, RooFit::LineColor(kBlack));
  RooHist* mom_pull = plot_mom->pullHist();
