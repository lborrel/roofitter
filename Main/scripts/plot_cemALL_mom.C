void plot_cemALL_mom(std::string filename) {
  ...
  pdf->plotOn(plot, RooFit::Components("RPCmomEffRes"), RooFit::LineColor(kGreen), RooFit::LineStyle(kDashed));
  pdf->plotOn(plot,RooFit::Components("crvFlatmomEffRes"),RooFit::LineColor(kCyan),RooFit::LineStyle(kDashed));
